var cedula, paciente,numNimbo,ddlChopo,ddlOrden;
...
function CargaSelect1(Command, Control) {
    var NameTable = "Tabla";
    var Lab = null;
    Lab = document.getElementById("cmbLab") ? $("#cmbLab").val() : null
    if (Lab != "") {
        App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });
        ...
        $.ajax({
            ...
            type: "POST",
            url: "/AjaxQuerys/AjaxResponse.aspx/" + Command,
            data: JSON.stringify({ "NombreTabla": Lab }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                Control.html('');
...
function SetRelacion()
{
    ...
    paciente = document.getElementById("PacienteRelacionNim") ? $("#PacienteRelacionNim").val() : null;
    cedula = document.getElementById("MedicoRelacionNim") ? $("#MedicoRelacionNim").val() : null;
    numNimbo = document.getElementById("NumeroRelacionNimbo") ? $("#NumeroRelacionNimbo").val() : null;

    $.ajax({
        cache: false,
        type: "POST",
        url: "/AjaxQuerys/AjaxResponse.aspx/SetRelacionNimbo",
        data: JSON.stringify({ "Paciente": paciente, "Cedula":cedula, "numNimbo":numNimbo }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            App.unblockUI("#Entradas_Portlet");
            alert(response.d);
            window.location = "/Estudios/RelacionMedNim";
        },
        failure: function (response) {
            //b(Command, IdControl);
            alert("Falló la llamada");
...
function AgregaCargaDetalleJS()
{
    var encontrado = false;
    var numChopo = document.getElementById("NumChopo") ? $("#NumChopo").val() : null;
    var texto = $(this).find('option:selected').text();
    var DescClave = document.getElementById("DescEstudio") ? $("#DescEstudio").find('option:selected').text(): null;
    var Clave = document.getElementById("ClaveEstudio") ? $("#ClaveEstudio").val() : null;
    if (Clave != "")
    {
        ...
        document.getElementById("DescEstudio").value = "";
        document.getElementById("ClaveEstudio").value = "";

    }
}

async function GuardarCargaDetalleJS() {
    var encontrado = false;
    var numChopo;
    var DescClave;
    var Clave;
    ...
    Resultado = "";
        ...
        if (datas != null) {
            ...
            filas.each(function (value, index) {
                ...
                $.ajax({
                    cache: false,
                    type: "POST",
                    url: "/AjaxQuerys/AjaxResponse.aspx/SetDetalleChopoEstudios",
                    data: JSON.stringify({ "NumChopo": numChopo, "Clave": clave, "DescClave": DescClave }),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (response) {
                        numFilas = numFilas - 1;
                        ...
                        {
                            if ((response.d) != "INSERCION EXITOSA")
                            {
                                Resultado = response.d;
                            }
                        }
                        ...
                        {
                            alert(Resultado);
                            window.location = "/Estudios/CargaDetalleEstudio";
                        }
        ...
        {
            alert("Tiene que agregar almenos una clave para guardar");
        }

    
}

function VerCargaDetalleChopo(Control, DataColumns, NumColumn) {
    var nameTable = "Tabla";
    ...
    Control.DataTable({
        ...
        ajax:
        {
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: "/AjaxQuerys/AjaxResponse.aspx/VerDetalleCargaChopo",
            dataSrc: "d",
            data: function (data) {
                return JSON.stringify({ "NombreTabla": nameTable });
            }
        },
        columns: DataColumns,
        dom: domin,
...
function AgregaChopoRelacion()
{
    ddlChopo = null;
    ddlOrden = null;

    ddlChopo = document.getElementById("SolicitudChopo") ? $("#SolicitudChopo").val() : null;
    ddlOrden = document.getElementById("ordenChopo") ? $("#ordenChopo").val() : null;

    var t = $('#TableConfirmacionChopo').DataTable();
    ...
    if (datas != null)
    {
        $("input:checkbox:checked").each(function () {
            numFilas = numFilas + 1;
            // alert($(this).val());
           // var nuevo = valores.push($(this).val());
        });
        var filas = t.rows().data();

        var valores = new Array(numFilas);
        var ClavesMal = new Array(numFilas);
        $("input:checkbox:checked").each(function () {
            valores[cuenta] = $(this).val();
            var fila = $(this).parents("tr");
            var cm = fila.find(".ClaveMal").val();
            ClavesMal[cuenta] = cm;
            cuenta = cuenta + 1;
        });
    ...
    if ((ddlChopo != "") && (ddlOrden != ""))
    {

        $.ajax({
            cache: false,
            type: "POST",
            url: "/AjaxQuerys/AjaxResponse.aspx/SetRelacionSolucionOrden",
            data: JSON.stringify({ "numchopo": ddlChopo, "OrdenChopo": ddlOrden, "detalle": valores, "clavemal": ClavesMal }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                ...
                App.unblockUI("#Entradas_Portlet");
                alert(response.d);
                window.location = "/Estudios/ConfirmacionResultados";
    ...
    {
        alert("Debe registrar un número de Solicitud y un número de Orden Chopo para poder guardar. Intentelo nuevamente.");
    }
}


function AgregaCargaDetalleNimbo() {
    var encontrado = false;
    var numChopo = document.getElementById("NumNimbo") ? $("#NumNimbo").val() : null;
    var texto = $(this).find('option:selected').text();
    var DescClave = document.getElementById("DescEstudioNimbo") ? $("#DescEstudioNimbo").find('option:selected').text() : null;
    var Clave = document.getElementById("ClaveEstudioNimbo") ? $("#ClaveEstudioNimbo").val() : null;
    var precio = document.getElementById("PrecioEstudioNimbo") ? $("#PrecioEstudioNimbo").find('option:selected').text()  : null;
    var Total = 0;
    if (Clave != "") {
        ...
        filas.each(function (value, index) {
            ...
            $('#TableCargaDetalleNimbo tfoot tr th').eq(1).text(Total);

        document.getElementById("PrecioEstudioNimbo").value = "";
        document.getElementById("DescEstudioNimbo").value = "";
        document.getElementById("ClaveEstudioNimbo").value = "";

    }
}


async function GuardarCargaDetalleNimboJS() {
    var encontrado = false;
    var numChopo;
    var DescClave;
    var Clave;
    ...
    if (datas != null) {
        ...
        filas.each(function (value, index) {
            ...
            $.ajax({
                cache: false,
                type: "POST",
                url: "/AjaxQuerys/AjaxResponse.aspx/SetDetalleNimboEstudios",
                data: JSON.stringify({ "NumNimbo": numChopo, "Clave": clave,"Lab":Lab }),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (response) {
                    numFilas = numFilas - 1;
                    ...
                    if (numFilas == 0) {
                        alert(Resultado);
                        window.location = "/Estudios/CargaDetalleNimbo";
                    }
...
function VerCargaDetalleNimbo(Control, DataColumns, NumColumn) {
    var NumNimbo = document.getElementById("NumNimboPdf") ? $("#NumNimboPdf").val() : null;
    if ($(window).width() <= 1199) {
        resp = 0;
        domin = "<'top'lf>rt<'bottom'ip><'clear'>";
        $("#Contenedor").css('top', '0px');
    ...
    Control.DataTable({
        ...
        ajax:
        {
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: "/AjaxQuerys/AjaxResponse.aspx/VerDetalleCargaNimbo",
            dataSrc: "d",
            data: function (data) {
                return JSON.stringify({ "NombreTabla": NumNimbo });
            }
        },
        columns: DataColumns,
        dom: domin,
...
function VerDetalleCargaNimboFiltro()
{
    CambiaLaboratorioPdf();
...
function CambiaLaboratorioPdf()
{
    var NumNimbo = document.getElementById("NumNimboPdf") ? $("#NumNimboPdf").val() : null;
    $.ajax({
        cache: false,
        type: "POST",
        url: "/AjaxQuerys/AjaxResponse.aspx/GetLabPdf",
        data: JSON.stringify({ "NumNimbo": NumNimbo }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            if (NumNimbo != "") {
                var idlab = null;
                idlab = response.d;
                $("#LabPdf").val(idlab).trigger('change.select2');
            }
            else
            {
                $("#LabPdf").val(null).trigger('change.select2');
            }
        },
        failure: function (response) {
            //b(Command, IdControl);
    ...
    $.ajax({
        cache: false,
        type: "POST",
        url: "/AjaxQuerys/AjaxResponse.aspx/GetFechaIniPdf",
        data: JSON.stringify({ "NumNimbo": NumNimbo }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            if (NumNimbo != "") {
                ...
                if (Fecha != "")
                {
                    document.getElementById("dprFechaValidez").setAttribute("disabled", "disabled");
                    document.getElementById("dprFechaValidez").value = Fecha;
                }
                else
                {
                    document.getElementById("dprFechaValidez").removeAttribute("disabled");
                    //document.getElementById("NumNimboPdf").value = "";
                }
            }
        },
        failure: function (response) {
...
function OcultaBotonDescargaPase()
{
    var idLab = document.getElementById("LabPdf") ? $("#LabPdf").val() : null;
    if (idLab != "2") {
        document.getElementById("divBtnDescarga").setAttribute("hidden", "hidden");
        document.getElementById("divFechaValidez").setAttribute("hidden", "hidden");
    }
    else
    {
        document.getElementById("divBtnDescarga").removeAttribute("hidden");
        document.getElementById("divFechaValidez").removeAttribute("hidden");
    }
}
function DescargaPase()
{
    var NumNimbo = document.getElementById("NumNimboPdf") ? $("#NumNimboPdf").val() : null;
    var FechaValidez = document.getElementById("dprFechaValidez") ? $("#dprFechaValidez").val() : null;;
    if (FechaValidez != "") {
        var direc = '/Views/Estudios/Descarga.aspx?lvFile=' + NumNimbo+"&&fvalid="+FechaValidez;
        window.location = direc;
    }
    else
    {
        alert("Debe colocar la fechade validez");
    }
    //window.open(direc, 'popup_window', 'width = 840, height = 420, left = 1, top = 1, resizable = yes, scrollbars = yes');
}
