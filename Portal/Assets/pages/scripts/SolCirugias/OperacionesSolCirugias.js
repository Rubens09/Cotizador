...
{
    CargaCombosSolCiru($("#cmbHospSolCir"), "0");
    CargaCombosSolCiru($("#cmbCirSolCir"), "1");
    CargaCombosSolCiru($("#cmbCausaSolCir"), "2");
    CargaCombosSolCiru($("#cmbPacienteSolCir"), "3");
    CargaCombosSolCiru($("#cmbDiagnosticos"), "4");
    CargaCombosSolCiru($("#cmbCpts"), "5");
    CargaCombosSolCiru($("#cmbMedicoSolCir"), "6");
    CargaCombosSolCiru($("#cmbTipSolicitud"), "7");
    var t = $('#TableDiag').DataTable({
        "sort": true,
        "aaSorting": [],
        "columnDefs": [{
            "targets": 0,
...
function CargaCombosSolCiru(Control,IdCatalogo)
{
    App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });
    $.ajax({
        cache: false,
        type: "POST",
        url: "/AjaxQuerys/AjaxResponse.aspx/GetCombosSolCirugia",
...
function AgregaDiagnostico()
{
    var encontrado = false;
    var Cie10 = document.getElementById("cmbDiagnosticos") ? $("#cmbDiagnosticos").val() : null;
    var texto = $(this).find('option:selected').text();
    var DescCie10 = document.getElementById("cmbDiagnosticos") ? $("#cmbDiagnosticos").find('option:selected').text() : null;
        ...
        if (Cie10 != "") {
            ...
            document.getElementById("cmbDiagnosticos").value = "";
...
{
    var encontrado = false;
    var Cpt = document.getElementById("cmbCpts") ? $("#cmbCpts").val() : null;
    var texto = $(this).find('option:selected').text();
    var DescCpt = document.getElementById("cmbCpts") ? $("#cmbCpts").find('option:selected').text() : null;

    if (Cpt != "") {
        var t = $('#TableCpt').DataTable();
        var datas;
        ...
        document.getElementById("cmbCpts").value = "";
        //document.getElementById("ClaveEstudio").value = "";
...
{
    var Cedula = document.getElementById("cmbMedicoSolCir") ? $("#cmbMedicoSolCir").val() : null;
    document.getElementById("txtCedulaMedicoTratante").value = "";
    document.getElementById("txtEspecialidadMedicoTratante").value = "";
    document.getElementById("txtTelefonoCiru").value = "";
    if (Cedula != "")
    {
        App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });
        $.ajax({
            cache: false,
            type: "POST",
            url: "/AjaxQuerys/AjaxResponse.aspx/GetInfoSolCirMed",
            ...
            success: function (response) {
                ...
                $.each(response.d, function (id, option) {
                    document.getElementById("txtEspecialidadMedicoTratante").value = option.Id;
                    document.getElementById("txtTelefonoCiru").value = option.Name;
                    document.getElementById("txtCedulaMedicoTratante").value = Cedula;
                });
            },
            failure: function (response) {
                //b(Command, IdControl);
...
function limpiarSolCiru()
{
    window.location = "/Gestion/SolicitudCirugia";
}

function GuardarSolCir()
{
    App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });
    var afilia, NombreResp, ApellidoPatResp, ApellidoMatResp, TipoIngre, Hospital, Episodio, Causa, Cedula,tipSol,FechaEstima;
    Cedula = document.getElementById("txtCedulaMedicoTratante").value;
    Causa = document.getElementById("cmbCausaSolCir").value;
    Episodio = document.getElementById("txtEpisodioSolCir").value;
    Hospital = document.getElementById("cmbHospSolCir").value;
    TipoIngre = document.getElementById("cmbCirSolCir").value;
    ApellidoMatResp =null;
    ApellidoPatResp = null;
    NombreResp = null;
    afilia = document.getElementById("cmbPacienteSolCir").value;
    tipSol = document.getElementById("cmbTipSolicitud").value;
    FechaEstima = document.getElementById("dprFechaEstimadaSol").value;
    ...
    if (datas != null) {
        ...
        var CodCie10 = new Array(numFilas);
        
        filas.each(function (value, index) {
            CodCie10[cuenta] = value[0];
            cuenta = cuenta + 1;
    ...
    if (encontrado == true)
    {
        ...
        {
            alert("Tiene que agregar almenos un CPT");
            encontrado = false;
        }
    }
    if (encontrado == true)
    {
        if (Cedula != "" && Causa != "" && Hospital != "" && TipoIngre != "" && ApellidoMatResp != "" && ApellidoPatResp != "" && NombreResp != "" && afilia != "")
        {
           
            $.ajax({
                cache: false,
                type: "POST",
                url: "/AjaxQuerys/AjaxResponse.aspx/AgregaCabSolCir",
                data: JSON.stringify({ "afilia": afilia, "NombreResp": NombreResp, "ApellidoPatResp": ApellidoPatResp, "ApellidoMatResp": ApellidoMatResp, "TipoIngre": TipoIngre, "Hospital": Hospital, "Episodio": Episodio, "Causa": Causa, "Cedula": Cedula, "Diag": CodCie10, "cpt": cpt, "tipSol": tipSol, "FechaEstima": FechaEstima }),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (response)
                {
...
function SeleccionCiruProc()
{
    var TipoSol = document.getElementById("cmbTipSolicitud").value;
    $("#cmbHospSolCir").select2("val", "");
    $("#cmbCausaSolCir").select2("val", "");
    if (TipoSol==2)
    {
        document.getElementById("cmbHospSolCir").setAttribute("disabled", "disabled");
        document.getElementById("cmbCausaSolCir").setAttribute("disabled", "disabled");
        $("#cmbHospSolCir").val("CONSUL").trigger('change.select2');
    }
    else
    {
        document.getElementById("cmbHospSolCir").removeAttribute("disabled");
        document.getElementById("cmbCausaSolCir").removeAttribute("disabled");
    }
}

function CargaCabeceroSolCir(Command, Control, DataColumns, NumColumn) {
    var Solicitud = "";
    ...
    Control.DataTable({
        ...
        ajax:
        {
            ...
            dataSrc: "d",
            data: function (data) {
                return JSON.stringify({ "IdSol": Solicitud });
            }
        },
        columns: DataColumns,
        dom: domin,
...
function CargaDetalleSolCir(Command, Control, DataColumns, NumColumn) {
    var Solicitud = document.getElementById("txtSolCir") ? $("#txtSolCir").val() : null;;
...
function DescargarSolicitudCir()
{
    var Folio = document.getElementById("txtSolCir") ? $("#txtSolCir").val() : null;
   
    if (Folio != "") {
        var direc = '/Views/Reportes/DescargaRep.aspx?lvFile=' + Folio ;
        window.location = direc;
    }
    else {
        alert("No exixte un folio valido");
    }
}
