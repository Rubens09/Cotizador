var ddlPaciente,ddlTipo,ddlEspecialidad,ddlMedico,ddlEpisodio,ddlFecha,ddlServicio,ddlMotivo,ddlDiagnostico,ddlTelefono,ddlNota,ddlFechaAlta,ddlInterconsulta,ddlMonto;
var ddlFechaDefuncion, ddlDefuncion, ddlHospitalizado,ddlCovid,ddlFechaIngreso,ddltipoInterConsulta,ddlNomPac,ddlIdRegion,ddlRegion,ddlPermiso;

function CambiaServicio(pagina)
{
    if (pagina == 1)
    {
        ddlPaciente = null;
        ddlPaciente = document.getElementById("pacienteId") ? $("#pacienteId").val() : null;
        ddlEpisodio = "";
        ddlMotivo = "";
        ddlServicio = "";
        ddlNomPac = null;
        ddlNomPac = document.getElementById("pacienteId") ? $("#pacienteId").find('option:selected').text() : null;
        ddlPermiso = null;
        ddlPermiso = document.getElementById("PermisoId") ? $("#PermisoId").val() : null;
        ddlIdRegion = null;
        if (ddlPermiso == "0") {
            ddlIdRegion = document.getElementById("RegionId") ? $("#RegionId").val() : null;
            ddlRegion = document.getElementById("RegionId") ? $("#RegionId").find('option:selected').text() : null;
        }
        else {
            ddlIdRegion = document.getElementById("RegionHideId") ? $("#RegionHideId").val() : null;
            ddlRegion = document.getElementById("txtRegion") ? $("#txtRegion").val() : null;
        }
        if (ddlPaciente == "" || ddlIdRegion == "") {
            alert("Debe seleccionar un paciente y una region de la serie de opciones");
        }
        else {
            window.location = "/Pacientes/MedicoEstudio?Pacientes=" + ddlPaciente + "&Episodio=" + ddlEpisodio + "&Motivo=" + ddlMotivo + "&Hospital=" + ddlServicio + "&IDRegion=" + ddlIdRegion + "&NomPac=" + ddlNomPac + "&Region=" + ddlRegion;
        }
    }
    else
    {
        ddlPermiso = null;
        ddlPermiso = document.getElementById("permisoHidden") ? $("#permisoHidden").val() : null;
        ddlNomPac = null;
        ddlNomPac = document.getElementById("pacienteVerId") ? $("#pacienteVerId").val() : null;
        ddlEpisodio = null;
        ddlEpisodio = document.getElementById("episodioVerId") ? $("#episodioVerId").val() : null;
        ddlMotivo = null;
        ddlMotivo = document.getElementById("motivoVerId") ? $("#motivoVerId").val() : null;
        ddlServicio = null;
        ddlServicio = document.getElementById("hospitalVerId") ? $("#hospitalVerId").val() : null;
        ddlRegion = null;
        ddlRegion = document.getElementById("regionVerIngre") ? $("#regionVerIngre").val() : null;
        ddlIdRegion = null;

        $.ajax({
            cache: false,
            type: "POST",
            url: "/AjaxQuerys/AjaxResponse.aspx/GetRegionPacienteIngresados",
            data: JSON.stringify({ "Region": ddlRegion,"Paciente":ddlNomPac,"Episodio":ddlEpisodio }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                $.each(response.d, function (id, option) {
                    ddlIdRegion = option.Id;
                    ddlPaciente = option.Name;
                });
                App.unblockUI("#Entradas_Portlet");
                if (ddlPaciente == "" || ddlIdRegion == "") {
                    alert("Debe seleccionar un paciente y una region de la serie de opciones");
                }
                else {
                    window.location = "/Pacientes/MedicoEstudio?Pacientes=" + ddlPaciente + "&Episodio=" + ddlEpisodio + "&Motivo=" + ddlMotivo + "&Hospital=" + ddlServicio + "&IDRegion=" + ddlIdRegion + "&NomPac=" + ddlNomPac + "&Region=" + ddlRegion;
                }
            },
            failure: function (response) {
                //b(Command, IdControl);
                alert("Falló la llamada");
            }
        });

    }
   
}

function CargaPacienteIngreso()
{
    App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });
    var Afiliacion = document.getElementById("AfiliacionPac") ? $("#AfiliacionPac").val() : null;
    var Region = document.getElementById("AfiliacionPac") ? $("#AfiliacionPac").val() : null;
    $.ajax({
        cache: false,
        type: "POST",
        url: "/AjaxQuerys/AjaxResponse.aspx/GetPaciente" ,
        data: JSON.stringify({ "Afiliacion": NameTable }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            ddlServicio = document.getElementById("hiddenServicio") ? $("#hiddenServicio").val() : null;
            Control.html('');
            Control.append($('<option></option>').val("").html("Select"));
            $.each(response.d, function (id, option) {
                Control.append($('<option></option>').val(option.Id).html(option.Name));
            });
            if (Control1 == "servicio") {
                ValidaHospital();
            }
            App.unblockUI("#Entradas_Portlet");
        },
        failure: function (response) {
            //b(Command, IdControl);
            alert("Falló la llamada");
        }
    });


}

function CambiaIngreso()
{
    ddlPaciente = null;
    ddlPaciente = document.getElementById("pacienteId") ? $("#pacienteId").val() : null;

    window.location = "";
}
function validaEpisodio() {
    ddlEpisodio = null;
    ddlEpisodio = document.getElementById("episodioId") ? $("#episodioId").val() : null;

    if (ddlEpisodio != "") {
        document.getElementById("episodioId").setAttribute("readonly", "readonly");
        document.getElementById("motivoId").setAttribute("readonly", "readonly");
    }
    else {
        document.getElementById("episodioId").removeAttribute("readonly");
        document.getElementById("motivoId").removeAttribute("readonly");
        document.getElementById("servicioId").removeAttribute("disabled");
    }
}

function ValidaHospital() {
    var hospi = document.getElementById("hiddenServicio").value;
    if (hospi != "") {
        $("#servicioId").val(hospi).trigger('change.select2');
        document.getElementById("servicioId").setAttribute("disabled", "disabled");
        }
  
    }
function CargaSelect(Command,Control,Control1)
{
    Control.select2("val", "");
    var NameTable = "Tabla";
    App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });
    ddlIdRegion = document.getElementById("IDRegionIngre") ? $("#IDRegionIngre").val() : null;
    $.ajax({
        cache: false,
        type: "POST",
        url: "/AjaxQuerys/AjaxResponse.aspx/" + Command,
        data: JSON.stringify({ "IDRegion": ddlIdRegion }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            ddlServicio = document.getElementById("hiddenServicio") ? $("#hiddenServicio").val() : null;
            Control.html('');
                Control.append($('<option></option>').val("").html("Select"));
            $.each(response.d, function (id, option) {
                Control.append($('<option></option>').val(option.Id).html(option.Name));
            });
            if (Control1=="servicio") {
                ValidaHospital();
            }
            App.unblockUI("#Entradas_Portlet");
        },
        failure: function (response) {
            //b(Command, IdControl);
            alert("Falló la llamada");
        }
    });
}

function CambiaTipo()
{
    ddlTipo = null;
    ddlTipo = document.getElementById("tipoId") ? $("#tipoId").val() : null;
    if (ddlTipo == 1) {
        document.getElementById("divStaff").setAttribute("hidden", "hidden");
        document.getElementById("divSoluglob").removeAttribute("hidden");
    }
    else {
        document.getElementById("divStaff").removeAttribute("hidden");
        document.getElementById("divSoluglob").setAttribute("hidden", "hidden");
        //medicoStaffId
        document.getElementById("medicoStaffId").value = "STAFF URGENCIAS";
        CargaEspecialidad();
    }

}
function CargaEspecialidad()
{
    ddlTipo = null;
    ddlTipo = document.getElementById("tipoId") ? $("#tipoId").val() : null;

    ddlMedico = null;
    if (ddlTipo == "1")
        ddlMedico = document.getElementById("medicoId") ? $("#medicoId").val() : null;
    else
        ddlMedico = "0";

    ddlIDRegion = null;
    ddlIDRegion = document.getElementById("IDRegionIngre") ? $("#IDRegionIngre").val() : null;

    var NameTable = "Tabla";
   
      
        App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });

        $.ajax({
            cache: false,
            type: "POST",
            url: "/AjaxQuerys/AjaxResponse.aspx/GetEspecialidad",
            data: JSON.stringify({ "Cedula": ddlMedico, "IDRegion": ddlIDRegion, "Tipo": ddlTipo }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                $("#especialidadId").html('');
                $("#especialidadId").append($('<option></option>').val("").html("Select"));
                $.each(response.d, function (id, option) {
                    $("#especialidadId").append($('<option></option>').val(option.Id).html(option.Name));
                });
                App.unblockUI("#Entradas_Portlet");
            },
            failure: function (response) {
                //b(Command, IdControl);
                alert("Falló la llamada");
            }
        });
    

}

function CargaTelefono() {
    ddlTipo = null;
    ddlTipo = document.getElementById("tipoId") ? $("#tipoId").val() : null;
    ddlEspecialidad = null;
    ddlEspecialidad = document.getElementById("especialidadId") ? $("#especialidadId").val() : null;
    ddlMedico = null;
    ddlMedico = document.getElementById("medicoId") ? $("#medicoId").val() : null;

    if (ddlTipo == 1) {
        App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });

        $.ajax({
            cache: false,
            type: "POST",
            url: "/AjaxQuerys/AjaxResponse.aspx/GetTelefono",
            data: JSON.stringify({ "Especialidad": ddlEspecialidad,"Medico":ddlMedico }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {

                $.each(response.d, function (id, option) {
                    document.getElementById("telefonoId").value = option.Telefono;
                    //$("#especialidadId").val(option.Especialida).trigger('change.select2');
                });
                

                App.unblockUI("#Entradas_Portlet");
            },
            failure: function (response) {
                //b(Command, IdControl);
                alert("Falló la llamada");
            }
        });
    }
}


function GuardarIngreso()
{
    ddlPaciente = null;
    ddlTipo = null;
    ddlEspecialidad = null;
    ddlMedico = null;
    ddlEpisodio = null;
    ddlFecha = null;
    ddlServicio = null;
    ddlMotivo = null;
    ddlDiagnostico = null;
    ddlTelefono = null;
    ddlFecha = null;
    ddlFechaAlta = null;
    ddlInterconsulta = null;
    ddlHospitalizado = null;
    ddlFechaIngreso = null;
    ddltipoInterConsulta = null;
    ddlIdRegion = null;

    ddlPaciente = document.getElementById("AfiliacionPac") ? $("#AfiliacionPac").val() : null;
    ddlTipo = document.getElementById("tipoId") ? $("#tipoId").val() : null;
    ddlEspecialidad = document.getElementById("especialidadId") ? $("#especialidadId").val() : null;
    ddlEpisodio = document.getElementById("episodioId") ? $("#episodioId").val() : null;
    ddlFecha = $("#dprFrom").val();
    ddlFechaAlta = $("#dprAlta").val();
    ddlServicio = document.getElementById("servicioId") ? $("#servicioId").val() : null;
    ddlMotivo = document.getElementById("motivoId") ? $("#motivoId").val() : null;
    ddlDiagnostico = document.getElementById("diagnosticoId") ? $("#diagnosticoId").val() : null;
    ddlTelefono = document.getElementById("telefonoId") ? $("#telefonoId").val() : null;
    ddlNota = document.getElementById("notaId") ? $("#notaId").val() : null;
    ddlInterconsulta = $('#chkInterConsult').prop('checked') ? "1" : "null";
    ddlHospitalizado = $('#chkHospUrg').prop('checked') ? "1" : "null";
    ddltipoInterConsulta = document.getElementById("cmbTipoInterCon") ? $("#cmbTipoInterCon").val() : null;
    ddlIdRegion = document.getElementById("IDRegionIngre") ? $("#IDRegionIngre").val() : null;


    if (ddlTipo != "") {
        if (ddlTipo == 1) {
            ddlMedico = document.getElementById("medicoId") ? $("#medicoId").val() : null;
        }
        if (ddlTipo == 2) {
            ddlMedico = document.getElementById("medicoStaffId") ? $("#medicoStaffId").val() : null;
        }
        
        if (ddlPaciente != "") {
            if (ddlEpisodio != "") {
                if (ddlServicio != "") {
                    if (ddlMotivo != "") {
                        if (ddlDiagnostico != "") {
                            if (ddlEspecialidad != "") {
                                if (ddlMedico!="") {
                                    if (ddlTelefono != "") {
                                        
                                            Guardado();
                                        
                                    }
                                    else {
                                        alert("Debe Ingresar un número de telefono");
                                    }
                                }
                                else {
                                    alert("Falta ingresar el Médico");
                                }

                            }
                            else {
                                alert("Seleccione la especialidad");
                            }
                        }
                        else {
                            alert("Digite el Diagnostico del paciente");
                        }
                    }
                    else {
                        alert("Digite el motivo del ingreso");
                    }
                }
                else {
                    alert("Seleccione el proveedor de servicio");
                }
            }
            else {
                alert("Digite el episodio del paciente");
            }

        }
        else {
            alert("Elija Un paciente");
            window.location = "/";
        }


    }
    else
    {
        alert("Seleccione un tipo admitido");
    }
    

}
function Guardado() {
    App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });

    $.ajax({
        cache: false,
        type: "POST",
        url: "/AjaxQuerys/AjaxResponse.aspx/SetGuardaConsulta",
        data: JSON.stringify({
            "paciente": ddlPaciente, "cedula": ddlMedico, "servicio": ddlServicio, "tipo": ddlTipo, "fecha": ddlFecha, "motivo": ddlMotivo, "diagnostico": ddlDiagnostico, "Especialidad": ddlEspecialidad, "telefono": ddlTelefono, "nota": ddlNota, "episodio": ddlEpisodio, "alta": ddlFechaAlta,"interConsulta" : ddlInterconsulta,"Hospitalizado":ddlHospitalizado,"tipoInter": ddltipoInterConsulta,"IDRegion":ddlIDRegion  }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            alert("Guardado Exitosamente");
            window.location = "/Pacientes/VerIngresos";
        },
        failure: function (response) {
            //b(Command, IdControl);
            alert("Falló la llamada");
        }
    });

}

function ActualizaAlta() {
    var result1;
    var result;

    ddlFechaAlta = $("#dprVerAlta").val();
    ddlPaciente = document.getElementById("pacienteVerId") ? $("#pacienteVerId").val() : null;
    ddlEpisodio = document.getElementById("episodioVerId") ? $("#episodioVerId").val() : null;
    ddlServicio = document.getElementById("hospitalVerId") ? $("#hospitalVerId").val() : null;
    ddlMonto = document.getElementById("montoVer") ? $("#montoVer").val() : null;
    ddlDefuncion = $('#chkDefuncion').prop('checked') ? "1" : "0";
    ddlHospitalizado = $('#chkHospitalizado').prop('checked') ? "1" : "null";
    ddlCovid = $('#chkCovid').prop('checked') ? "1" : "0";

    ddlFechaIngreso = null;
    ddlFechaIngreso = document.getElementById("FechaIngresoOculta") ? $("#FechaIngresoOculta").val() : null;
    ddlFechaDefuncion = null;
    if (ddlDefuncion == "1")
    {
        
        ddlFechaDefuncion = document.getElementById("dprDefuncion") ? $("#dprDefuncion").val() : null;
        if (ddlFechaDefuncion == "")
        {
            alert("Si selecciona una defunción debe de inidicar la fecha de defunción");
            ddlDefuncion = "0";
        }
    }
  
    if (ddlFechaAlta != "")
    {
        result1 = confirm("¿Desea tomar la fecha de alta para actualizar?");
        if (result1 == true)
        {
            result = confirm("Estas seguro de dar de alta este paciente, una vez dado de alta no se podra realizar ninguna modificación");
            if (result == true)
            {
                ddlFechaAlta = $("#dprVerAlta").val();
            }
            else
            {
                ddlFechaAlta = null;
            }
        }
        else {
            ddlFechaAlta = null;
            document.getElementById("hospitalVerId").value = "";
        }
    }
    
    if (ddlFechaAlta != "") {
        var alt = ddlFechaAlta.split("/");
        var ing = ddlFechaIngreso.split("/");
        var altaF = new Date(alt[2], alt[1], alt[0]);
        var ingF = new Date(ing[2], ing[1], ing[0]);

        if (altaF < ingF) {
            alert("La fecha de alta no debe ser menor a la de ingreso.Intente nuevamente");
        }
        else {
            var final = confirm("Se realizará la actualización,¿Estas de acuerdo?");
            if (final == true) {
                App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });

                $.ajax({
                    cache: false,
                    type: "POST",
                    url: "/AjaxQuerys/AjaxResponse.aspx/SetAltaIngresados",
                    data: JSON.stringify({ "paciente": ddlPaciente, "ep": ddlEpisodio, "servicio": ddlServicio, "alta": ddlFechaAlta, "monto": ddlMonto, "defuncion": ddlDefuncion, "fechaDefuncion": ddlFechaDefuncion, "Hospitalizado": ddlHospitalizado, "Covid": ddlCovid }),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (response) {
                        alert("Guardado Exitosamente");
                        window.location = "/Pacientes/VerIngresos";
                    },
                    failure: function (response) {
                        //b(Command, IdControl);
                        alert("Falló la llamada");
                    }
                });
            }
        }
    }
    else
    {
        var final = confirm("Se realizará la actualización,¿Estas de acuerdo?");
        if (final == true) {
            App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });

            $.ajax({
                cache: false,
                type: "POST",
                url: "/AjaxQuerys/AjaxResponse.aspx/SetAltaIngresados",
                data: JSON.stringify({ "paciente": ddlPaciente, "ep": ddlEpisodio, "servicio": ddlServicio, "alta": ddlFechaAlta, "monto": ddlMonto, "defuncion": ddlDefuncion, "fechaDefuncion": ddlFechaDefuncion, "Hospitalizado": ddlHospitalizado, "Covid": ddlCovid }),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (response) {
                    alert("Guardado Exitosamente");
                    window.location = "/Pacientes/VerIngresos";
                },
                failure: function (response) {
                    //b(Command, IdControl);
                    alert("Falló la llamada");
                }
            });
        }
    }

}

function VerCirugias(Command, Control, DataColumns, NumColumn) {
    var paciente = document.getElementById("pacienteVerId") ? $("#pacienteVerId").val() : null;;
    var episodio = document.getElementById("episodioVerId") ? $("#episodioVerId").val() : null;;
    if ($(window).width() <= 1199) {
        resp = 0;
        domin = "<'top'lf>rt<'bottom'ip><'clear'>";
        $("#Contenedor").css('top', '0px');
    } else {
        resp = 1
        domin = "<'top'lf>rt<'bottom'ip><'clear'>";

    };
    Control.DataTable().destroy();
    Control.DataTable({
        drawCallback: function (settings) {
            if (settings.jqXHR != null) {
                //b(Command);
            }
        },
        ajax:
        {
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: "/AjaxQuerys/AjaxResponse.aspx/" + Command,
            dataSrc: "d",
            data: function (data) {
                return JSON.stringify({ "Paciente": paciente, "Episodio": episodio });
            }
        },
        columns: DataColumns,
        dom: domin,
        language:
        {
            aria:
            {
                sortAscending: ": activar para ordenar la columna ascendente",
                sortDescending: ": activar para ordenar la columna descendente"
            },
            emptyTable: "No hay datos disponibles en la tabla",
            info: "Resultado: _START_ al _END_ de _TOTAL_ entradas",
            infoEmpty: "Entradas no encontradas",
            infoFiltered: "(filtrada de _MAX_ entradas totales)",
            lengthMenu: "_MENU_",
            search: "Buscar:",
            zeroRecords: "No se encontraron registros coincidentes",
            paginate:
            {
                previous: "Anterior",
                next: "Siguiente",
                last: "Último",
                first: "Primero"
            }
        },
        footerCallback: function (e, t, a, r, o) {
            var n = this.api(),
                c = function (e) {
                    return "string" == typeof e ? 1 * e.replace(/[\$,]/g, "") : "number" == typeof e ? e : 0
                };
            total = n.column(NumColumn).data().reduce(function (e, t) { return c(e) + c(t) }, 0),
                $(n.column(NumColumn).footer()).html(total)
        },
        buttons: [
            { extend: "print", className: "btn dark btn-outline" },
            { extend: "copy", className: "btn red btn-outline" },
            {
                extend: "pdf", className: "btn green btn-outline", orientation: 'landscape', customize: function (doc) {
                    doc.content.splice(1, 0, {
                        margin: [0, 0, 0, 12],
                        alignment: 'center',
                        image: 'data:image/jpeg;base64,'
                    });
                }
            },
            { extend: "excel", className: "btn yellow btn-outline " },
            { extend: "csv", className: "btn purple btn-outline " },
            { text: "Reload", className: "btn default", action: function (e, t, a, n) { alert("Custom Button") } }],
        pagingType: "numbers",
        responsive: !resp,
        order: [[1, "asc"]],
        lengthMenu: [[10, 25, 50, 100, -1], [10, 25, 50, 100, "Todo"]],
        pageLength: 50
    });
    $("#TableEntradas_tools > li > a.tool-action").unbind('click').bind("click", function () {
        var e = $(this).attr("data-action");
        Control.DataTable().button(e).trigger()
    });
}

function ActualizaDiagnostico() {
    var NameTable = "Tabla";
    var DiagnosticoActualiza = document.getElementById("txtDiagnostico") ? $("#txtDiagnostico").val() : null;
    var DiagnosticoPasado = document.getElementById("txtDiagnosticoOculto") ? $("#txtDiagnosticoOculto").val() : null;
    var paciente = document.getElementById("pacienteVerId") ? $("#pacienteVerId").val() : null;;
    var episodio = document.getElementById("episodioVerId") ? $("#episodioVerId").val() : null;;


    if (DiagnosticoActualiza != "") {

        App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });

        $.ajax({
            cache: false,
            type: "POST",
            url: "/AjaxQuerys/AjaxResponse.aspx/SetActualizaDiagnostico",
            data: JSON.stringify({ "Paciente": paciente, "Episodio": episodio, "Diagnostico": DiagnosticoPasado, "DiagnosticoActualiza": DiagnosticoActualiza }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                document.getElementById("txtDiagnostico").value = "";
                document.getElementById("txtDiagnosticoOculto").value = "";
                document.getElementById("EditarDiagnostico").setAttribute("hidden", "hidden");

                var DataColums = [{ data: "Fecha" }, { data: "MOTIVO" }, { data: "DIAGNOSTICO" }, { data: "MEDICO" }, { data: "ESPECIALIDAD" },
                { data: "TELEFONO" }, { data: "SERVICIO" }, { data: "NOTA" }, { data: "INTERCON" }, { defaultContent: "<button type='button' class='editar btn btn-primary' onclick='muestraEditarDiag(1)'><i class='fa fa-pencil-square-o'></i></button>" }];
                CargaDetEpisodio("GetConsultaDetIngresados", $("#TableVerEpisodio"), DataColums, 7, paciente, episodio);

                App.unblockUI("#Entradas_Portlet");
            },
            failure: function (response) {
                //b(Command, IdControl);
                alert("Falló la llamada");
            }
        });
    }
    else {
        alert("Debe colocar el diagnostico nuevo");
    }
}

function AgregaInterconsulta() {
    var NameTable = "Tabla";
    var MedicoInterconsulta = document.getElementById("txtMedicoIntercon") ? $("#txtMedicoIntercon").val() : null;
    var FechaIntercon = $("#dprInterConsulta").val();
    var Especialidad = "";
    var paciente = document.getElementById("pacienteVerId") ? $("#pacienteVerId").val() : null;
    var episodio = document.getElementById("episodioVerId") ? $("#episodioVerId").val() : null;
    var tipoInterConsulta = document.getElementById("TipoInterconsulta") ? $("#TipoInterconsulta").val() : null;//TipoInterconsulta

    if ((FechaIntercon != "") && (tipoInterConsulta!="")) {

        App.blockUI({ target: "#Entradas_Portlet", boxed: !0, message: "Procesando…" });

        $.ajax({
            cache: false,
            type: "POST",
            url: "/AjaxQuerys/AjaxResponse.aspx/SetGuardaFechaInterConsulta",
            data: JSON.stringify({ "Paciente": paciente, "Episodio": episodio, "Medico": MedicoInterconsulta, "Especialidad": Especialidad, "fechaInterconsulta": FechaIntercon,"tipoInter":tipoInterConsulta}),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                var Result=response.d;
               
                    document.getElementById("txtMedicoIntercon").value = "";
                    document.getElementById("txtEspecialidadIntercon").value = "";
                    document.getElementById("dprInterConsulta").value = "";
                document.getElementById("AgregarInterConsulta").setAttribute("hidden", "hidden");

                if (Result == true) {
                    App.unblockUI("#Entradas_Portlet");
                    alert("Agregada la interConsulta");
                    $('#myModal').modal('hide');
                }
                else
                {
                    App.unblockUI("#Entradas_Portlet");
                    alert("La Fecha de interconsulta ya fue agregada, por favor verificar la información");
                    $('#myModal').modal('hide');
                }
            },
            failure: function (response) {
                
                alert("Falló la llamada");
            }
        });
    }
    else {
        alert("Debe colocar una fecha de Interconsulta y un tipo valido");
    }
}