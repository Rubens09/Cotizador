var AjaxGestTract = function () {
    var NameTable, ddlIDRegion, ddlMarca, ddlTracto, ddlEstatus,ddlPermiso;
    var IsRequest = false;

    var a = function (IdControl, ReloadTable) {
        if (IsRequest == false) {
            IsRequest = true;
            if (document.getElementById("GestTractRegionId") && (IdControl == "dpRange")) {
                $("#GestTractRegionId").select2("val", "");
            }
            if (document.getElementById("GestMarcaTractId") && (IdControl == "dpRange") || (IdControl == "GestTractRegionId")) {
                $("#GestMarcaTractId").select2("val", "");
            }
            if (document.getElementById("GestTractTractoId") && (IdControl == "dpRange") || (IdControl == "GestTractRegionId") || (IdControl == "GestMarcaTractId")) {
                $("#GestTractTractoId").select2("val", "");
            }
            if (document.getElementById("GestTractEstatusId") && (IdControl == "dpRange") || (IdControl == "GestTractRegionId") || (IdControl == "GestMarcaTractId") || (IdControl == "GestTractTractoId")) {
                $("#GestTractEstatusId").select2("val", "");
            }
            //if (document.getElementById("EstatusId") && (IdControl == "dpRange") || (IdControl == "GestTractRegionId") || (IdControl == "GestMarcaTractId") || (IdControl == "CajaId") || (IdControl == "ClienteId")) {
            //    $("#EstatusId").select2("val", "");
            //}
            ...
            //if (ddlPermiso != "0") {
            //    document.getElementById("GestTractRegionId").setAttribute("disabled", "disabled");
            //    //document.getElementById("divInputRegion").removeAttribute("hidden");
            //}
        ...
        b = function (Command, IdControl, ReloadTable) {
            switch (Command) {
                case "Initialize":
                    if (document.getElementById("GestTractRegionId") && ((IdControl == "dpRange"))) {
                        c("GetGestRegion", $("#GestTractRegionId"), IdControl, ReloadTable);
                    }
                    else {
                        b("GetGestRegion", IdControl, ReloadTable);
                    }
                    break;
                case "GetGestRegion":

                    ddlIDRegion = document.getElementById("GestTractRegionId") ? $("#GestTractRegionId").val() : null;

                    if (document.getElementById("GestMarcaTractId") && ((IdControl == "dpRange") || (IdControl == "GestTractRegionId"))) {
                        c("GetGestMarca", $("#GestMarcaTractId"), IdControl, ReloadTable);
                    }
                    else {
                        b("GetGestMarca", IdControl, ReloadTable);
                    }
                    break;

                case "GetGestMarca":

                    ddlMarca = document.getElementById("GestMarcaTractId") ? $("#GestMarcaTractId").val() : null;

                    if (document.getElementById("GestTractTractoId") && ((IdControl == "dpRange") || (IdControl == "GestTractRegionId") || (IdControl == "GestMarcaTractId"))) {
                        c("GetGestTracto", $("#GestTractTractoId"), IdControl, ReloadTable);
                    }
                    else {
                        b("GetGestTracto", IdControl, ReloadTable);
                    }
                    break;

                case "GetGestTracto":

                    ddlTracto = document.getElementById("GestTractTractoId") ? $("#GestTractTractoId").val() : null;

                    if (document.getElementById("GestTractEstatusId") && ((IdControl == "dpRange") || (IdControl == "GestTractRegionId") || (IdControl == "GestMarcaTractId") || (IdControl == "GestTractTractoId"))) {
                        c("GetGestEstatus", $("#GestTractEstatusId"), IdControl, ReloadTable);
                    }
                    else {
                        b("GetGestEstatus", IdControl, ReloadTable);
                    }
                    break;
                case "GetGestEstatus":

                    ddlEstatus = document.getElementById("GestTractEstatusId") ? $("#GestTractEstatusId").val() : null;

                    if (ReloadTable == 1) {

                        var DataColums = [{ data: "Region" }, { data: "Tracto" }, { data: "Marca" }, { data: "Modelo" }, { data: "Placas" }, { data: "Estatus" }, { data: "UltimaActualizacion" }];
                        d("GetGestTablaTracto", $("#TableGestTracto"), DataColums, 6);
        ...
        c = function (Command, Control, IdControl, ReloadTable) {
            ...
            $.ajax({
                ...
                data: JSON.stringify({ "Region": ddlIDRegion, "Marca": ddlMarca, "IDTracto": ddlTracto, "Estatus": ddlEstatus }),
        ...
        d = function (Command, Control, DataColumns, NumColumn) {
            ...
            Control.DataTable({
                ...
                ajax:
                {
                    ...
                    dataSrc: "d",
                    data: function (data) {
                        return JSON.stringify({ "Region": ddlIDRegion, "Marca": ddlMarca, "IDTracto": ddlTracto, "Estatus": ddlEstatus });
                    }
                },
                columns: DataColumns,
                dom: domin,
...
App.isAngularJsApp() === !1 && jQuery(document).ready(function () { AjaxGestTract.init() });;
