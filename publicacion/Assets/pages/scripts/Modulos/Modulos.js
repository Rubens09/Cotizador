...
var ddlFechaDefuncion, ddlDefuncion, ddlHospitalizado,ddlCovid,ddlFechaIngreso,ddltipoInterConsulta;
...
function CambiaServicio(pagina)
{
    ...
    if (ddlPaciente == "") {
        alert("Debe seleccionar un paciente del combo");
    }
    else
    {
        window.location = "/Pacientes/MedicoEstudio?Pacientes=" + ddlPaciente +"&Episodio="+ ddlEpisodio+"&Motivo="+ ddlMotivo +"&Hospital="+ddlServicio;
    }
}
...
function CargaMedicos()
{
    ...
    if (ddlTipo==1)
    {
        ...
        $.ajax({
            cache: false,
            type: "POST",
            url: "/AjaxQuerys/AjaxResponse.aspx/GetMedicos",
            data: JSON.stringify({ "Especialidad": ddlEspecialidad }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                $("#medicoId").html('');
                $("#medicoId").append($('<option></option>').val("").html("Select"));
                $.each(response.d, function (id, option) {
                    $("#medicoId").append($('<option></option>').val(option.Id).html(option.Name));
                });
                App.unblockUI("#Entradas_Portlet");
            },
            failure: function (response) {
...
function GuardarIngreso()
{
    ...
    ddlPaciente = document.getElementById("pacienteIngresoId") ? $("#pacienteIngresoId").val() : null;
    ddlTipo = document.getElementById("tipoId") ? $("#tipoId").val() : null;
    ddlEspecialidad = document.getElementById("especialidadId") ? $("#especialidadId").val() : null;
    ddlEpisodio = document.getElementById("episodioId") ? $("#episodioId").val() : null;
